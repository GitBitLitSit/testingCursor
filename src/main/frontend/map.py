    ...
    def _build_base_figure(self) -> go.FigureWidget:
            ...
            for real_idx, tree_indices in support_by_corr.items():
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        legendrank=20,
                        customdata=sup_custom,
                        hovertemplate="Stützmast<br>X: %{x:.2f}<br>Y: %{y:.2f} <br>BHD: %{customdata[0]:.1f} cm<extra></extra>",
                        showlegend=False,
                    )
                )
        ...
        if not support_present:
            ...
            if isinstance(support_mask, (list, tuple)) and len(support_mask) == len(tree_x):
                ...
                sup_custom = [[tree_bhd[i]] for i, m in enumerate(support_mask) if m]
            else:
                sup_x, sup_y, sup_custom = [], [], []
