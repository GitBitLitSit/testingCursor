...
from src.main.frontend.data_prep import build_viz_data, get_side_profile_data
...
def build_interface(forest_area_3, model_list, results_df: pd.DataFrame) -> w.VBox:
    ...
    radar_chart = build_radar_dashboard(
        ...
        738,
        _NAMES["axes"],
        on_toggle=lambda idx, active: overview_table.unmute_row(idx)
        if active
        else overview_table.mute_row(idx),
    )
    ...
    def on_corridor_select(corridor_idx_in_list: int | None) -> None:
    ...
    def on_model_select(idx: int | None) -> None:
        ...
        if idx is None:
            ...
            custom_labels = [
                f"Seiltrasse {vd.real_to_display.get(int(rid), int(rid))}"
                for rid in real_ids
            ]
    ...
    sel_widget = selected_table.get_widget()
    anch_widget = anchor_table.get_widget()
