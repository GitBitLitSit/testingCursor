...
class Table:
    def __init__(
        ...
        self._has_action = (action_label is not None)
    ...
    def update_data(self, new_data: List[List[str]]) -> None:
        ...
        self._selected = -1
        # Re-apply muted state if needed (optional logic)
                    ...
                    def _cb(_):
                        ...
                        if self._highlighted_row == idx:
                            ...
                            if callable(self._on_action):
                                self._on_action(-1) # Send -1 to indicate "None selected"
                        else:
                            # Standard selection
